...
void daemonize(void)
{
	...
	if(getrlimit(RLIMIT_NOFILE,&rl) < 0)
		err_exit("getrlimit : cna't get file limit");
...
ls(int argc, char *argv[])
{
	...
	// if path is "ls -l /etc", argvoid connect_to_client()cnt is argv[2](/etc)
...
void connect_to_client()
{
	...
	if( (ifd != 0) || (ofd != 1))
		err_exit("ifd and ofd are not 0 and 1, respectively");
